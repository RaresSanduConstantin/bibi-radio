...
const Stations = ({ item }) => {
  ...
  const favoriteRadio = () => {
    if (!favorite) {
      ...
      localStorage.setItem("favoriteRadio", item.name);
    } else {
      setFavorite(false);
      item.isFavorite = false;
      localStorage.removeItem("favoriteRadio", item.name);
    }
  };
  ...
  return (
    <div>
      <div className="radioContainer">
        ...
        <div
          onClick={toggle}
          class={!playing ? "img-container" : "img-container play"}
        >
          <img
